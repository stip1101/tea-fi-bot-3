...
export default async function handleReportApproval(
  ...
  if (userDiscordId) {
    try {
      ...
      const dmEmbed = createReportDMEmbed(true, reportMonthYear, reportDocLink, reviewNotes);
      await discordUser.send({ embeds: [dmEmbed] });
      dmSent = true;
    } catch (error) {
      handlerLogger.warn({ err: error, userId: userDiscordId }, 'Failed to send report approval DM');
    }
  }

  let response = `${EMOJIS.CHECK} Report approved!`;
